...
func (g *Game) submitted_superword(word string) (bool, string) {
	for _, record := range g.Records {
		for _, otherWord := range record.Words {
			if is_substring(word, otherWord) {
				return true, otherWord
			}
		}
	}
	return false, ""
...
// Takes away half of the original points from players with words that are substring of the word, and removes those words from the player's record.
func (g *Game) steal_subwords(word string, p *Player) {
	var trimmedPlayerWords []string
	for _, record := range g.Records {
		if record.Player != p {
			trimmedPlayerWords = make([]string, 0)
			for _, otherWord := range record.Words {
				if is_substring(otherWord, word) {
					record.Player.Score -= g.word_pts(otherWord) / 2
				} else {
					trimmedPlayerWords = append(trimmedPlayerWords, otherWord)
				}
			}
			record.Words = trimmedPlayerWords
		}
	}
}
...
func (g *Game) submit_word(word string, p *Player) {
	...
	for otherWord, _ := range g.IsUsed {
		if otherWord == word {
			e := &event{
				Name: "wrong_because",
				Data: fmt.Sprintf("The sentence contains %v or someone already submitted it", word),
			}
			p.Send(e)
			return
		}
	...
	// Check if someone has already submitted a word that contains this word
	hasSuper, superword := g.submitted_superword(word)
	if hasSuper {
		e := &event{
			Name: "wrong_because",
			Data: fmt.Sprintf("%v contains %v", superword, word),
		}
		p.Send(e)
		return
	}

	// Remove prefix rules for the time being since they conflict
	...
	// Steal words that are shorter from other players
	g.steal_subwords(word, p)
